...
router.post('/loginWithProof', async (req, res) => {
  ...
  const { tempEphemeral, salt, verifier, encryptedKey, files } = await User.findOne({ email });

  try {
    const serverSession = srp.deriveSession(tempEphemeral, clientEphemeral, salt, email, verifier, clientProof);
    res.status(200).json({ serverProof: serverSession.proof, key: encryptedKey, files });
  } catch (err) {
